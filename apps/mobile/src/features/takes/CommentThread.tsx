...
function ReplyLink({ onPress, marginTop }: { onPress: () => void; marginTop: number }) {
  const { colors } = useTheme();
  return (
    <PressableOpacity onPress={onPress} style={{ alignSelf: "flex-start", marginTop, paddingVertical: 2 }}>
...
function ReplyRow({ reply, onReply }: { reply: TakeComment; onReply: () => void }) {
  const { colors } = useTheme();
  return (
    <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        ...
        <ReplyLink onPress={onReply} marginTop={4} />
      </View>
    </View>
  );
}

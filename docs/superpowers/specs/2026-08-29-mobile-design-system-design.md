...
1. **expo-router** — 파일 기반 라우팅. 라우트 파일은 얇은 래퍼로 두고
   화면 본체는 feature 폴더에 co-locate.
...
3. **Mock 구현체 포함** — `RehearsalApiClient` 인터페이스의 MockApiClient가
   ...
   인터페이스만 구현해 교체.
4. **도메인 co-location** — 화면·컴포넌트·훅을 도메인 폴더에 모은다.
...
apps/mobile/
  app/
    ...
    (tabs)/index.tsx               # → SessionsScreen
    (tabs)/band.tsx                # → BandScreen
    record.tsx                     # → RecordingScreen
    processing.tsx                 # → ProcessingScreen
    session/[id]/index.tsx         # → SessionDetailScreen
    session/[id]/take/[takeId].tsx # → TakePlayerScreen
  src/
    ...
    features/
      ...
      takes/      SessionDetailScreen, TakeRow, TakePlayerScreen,
                  CommentRow, CommentInput, usePlayback(모의)
      band/       BandScreen, MemberRow, BandSwitchSheet, InviteSheet

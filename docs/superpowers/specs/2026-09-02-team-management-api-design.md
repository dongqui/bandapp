...
팀 관리 화면이 요구하는 세 가지를 서버가 아직 제공하지 못한다.
...
이 셋을 채워 팀 관리 화면이 mock 없이 실제 서버로 동작하게 한다.
...
6. **초대 화면 진입마다 초대가 새로 발급되는 문제는 이번에 고치지 않는다.** `band_invites`는 `tokenHash`만 저장하고 평문 토큰을 갖고 있지 않아 "활성 초대 재사용"이 원리적으로 불가능하다. 하려면 평문을 저장해야 하는데 해시 저장 설계를 뒤집는 일이다.

   만료가 7일이고 결정 5의 revoke가 개수와 무관하게 동작하므로 실해가 없다. 고치려면 클라이언트를 "명시적 발급"으로 바꾸거나 토큰 저장 방식을 재검토해야 하며, 둘 다 별개 결정이다.
...
마이그레이션은 `drizzle-kit generate`로 뽑는다(`0002_*.sql`). 기존 행은 전부 `null`로 남고 백필은 없다 — 미설정이 정상 상태다.
...
| 상황 | 상태 | code | message |
|---|---|---|---|
| 토큰 해시 미존재 | 404 | `invite_not_found` | 유효하지 않은 초대예요. |
...
1. **초대 화면 진입마다 초대가 새로 발급된다** (결정 6). 고치려면 평문 토큰 저장을 재검토하거나 클라이언트를 명시적 발급으로 바꿔야 한다.
2. **모바일 화면 배선.** 파트 설정 시트, 팀원 관리 시트, 내보내기 확인, 초대 오류 4분기가 새 API를 쓰도록 연결하는 작업.
3. **소유권 이전.** `leave`가 owner에게 "소유권을 넘기거나 팀을 삭제해야 해요"라고 안내하지만 두 경로 모두 구현이 없다.
4. **`max_uses`를 쓰는 1회용 초대.** 분기는 준비돼 있으나 이를 발급하는 경로가 없다.

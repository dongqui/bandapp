...
재사용에는 잔여 수명 하한이 있다 — 만료까지 24시간 이상 남아야 재사용 대상이다. 그 미만이면 재사용하지 않는다. 리허설 룸 보면대에 붙여둔 QR이 화면을 다시 열자마자 몇 분 뒤 죽어버리는 링크로 바뀌는 걸 막기 위함이다(이 기능의 핵심 시나리오). 하한에 못 미친 초대는 revoke하지 않고 그냥 방치한다 — 어차피 하루 안에 스스로 만료되므로 명시적으로 무효화할 이유가 없고, 정상 상태(활성 초대 하나)를 유지하는 데 필요한 최소한의 개입만 한다.

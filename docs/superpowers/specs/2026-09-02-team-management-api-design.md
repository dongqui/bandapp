...
재사용에는 잔여 수명 하한이 있다 — 만료까지 **6시간** 이상 남아야 재사용 대상이다. 그 미만이면 재사용하지 않고 새로 발급한다. 합주실 보면대에 띄워둔 QR이 화면을 다시 열자마자 몇 분 뒤 죽는 링크로 바뀌는 걸 막기 위함이다(이 기능의 핵심 시나리오).

하한의 크기는 트레이드오프다. 하한에 걸리면 새 링크가 발급되고 옛 링크는 방치된 채 살아있으므로, 그 구간 동안 밴드에 살아있는 링크가 2개가 된다 — 재사용을 도입한 명분("밴드당 살아있는 링크 하나")을 그만큼 되돌리는 셈이다. 하한이 TTL에서 차지하는 비율이 곧 그 2개 구간의 비율이다: 7일 TTL에 6시간이면 3.5%다. 합주실 QR은 몇 분이면 되고, 링크를 보내고 상대가 여는 지연도 대개 몇 시간 안이라 6시간이면 덮인다. 24시간으로 잡으면 그 구간이 14%가 되어 비싸다.

하한에 못 미친 초대는 revoke하지 않고 방치한다 — 어차피 곧 스스로 만료되므로 명시적으로 무효화할 이유가 없다. 그리고 `REUSE_MIN_REMAINING_MS`는 `INVITE_TTL_DAYS`보다 충분히 작아야 한다. 둘이 가까워지면 모든 발급이 하한에 걸려 재사용이 사실상 죽는다.
